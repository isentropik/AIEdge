#pragma once
#include "MeterProfileJson.h"
#include "../jomjol_controlcamera/ConfigJournal.h"

namespace meter { namespace ProfileStore {
using ConfigJournal::Storage;
using ConfigJournal::Read;
using ConfigJournal::Result;
inline bool valid(const std::string& bytes){RegisterProfile profile;return parseProfile(bytes,profile);}
...
// never guess physical units or delete evidence that could be needed for recovery.
...
inline Result commit(Storage& disk,const std::string& path,bool existed,
    ...
    if(existed){
        RegisterProfile oldProfile,newProfile;
        if(!parseProfile(before,oldProfile)||!parseProfile(after,newProfile)||
           !samePhysicalScale(oldProfile,newProfile))return Result::Conflict;
    }
