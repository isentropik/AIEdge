...
Only the explicit static-site files are published to the separate `codex/aiedge-site` branch. GitHub Pages serves that branch over HTTPS. Source history, node_modules and local working files are not copied into the website. The pinned compiled installer and its license notices are published under vendor/. See tools/README.md for the reproducible build and USB discovery recovery change.
...
The main package is still downloaded and verified by the board after Wi-Fi setup. Version 0.1.1's loader retains the pinned 0.1.0 application package; only the loader and browser installation flow change. The user verified that USB Wi-Fi setup appears after a manual console reset. USB setup fix 1 automates one reset/retry on discovery failure; its unit and bundled-browser simulation checks pass, and the automatic hardware path awaits the user test. Full package installation remains separately unverified.
