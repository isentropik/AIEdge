...
$('retry').onclick=async()=>{busy=true;controls();$('retry').disabled=true;try{const r=await deviceFetch('/retry',{method:'POST'});if(!r.ok)state.textContent=await r.text();}catch(e){state.textContent='Connection interrupted. Checking device…';}};
async function poll(){try{const r=await deviceFetch('/status',{cache:'no-store'});if(!r.ok)throw Error();const s=await r.json();if(/^aiedge-[0-9a-f]{6}$/.test(s.hostname||'')){const address=$('device-address');address.textContent=s.hostname+'.local';address.href='http://'+s.hostname+'.local';address.hidden=false;$('address-pending').hidden=true;}busy=s.phase>0;state.textContent=s.message;$('progress').value=s.progress;const transfer=transferText(s);$('transfer').textContent=transfer.bytes;$('speed').textContent=transfer.speed;$('refresh').hidden=true;controls();
$('wifi').hidden=busy||s.wifi_connected;
$('intro').textContent=s.wifi_connected?'Wi-Fi connected':busy?'Reconnecting and continuing installation…':'Choose your home Wi-Fi to download and install AIEdge.';
