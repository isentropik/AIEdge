...
The setup page detects the application using its plain-text hostname endpoint, checks that the homepage responds, and opens it automatically. It retries while the device is offline or the page is unavailable. Host checks cover both outcomes and ensure polling never starts installation. A recorded 0.1.10 test-board installation completed download, SD verification, firmware write, restart and saved Wi-Fi reconnection. The browser opened the application automatically without a refresh. The homepage, hostname and system-information checks passed, and the USB log showed no abort, stack overflow or route-capacity error. The disconnected camera produced the expected error 0x00000004; camera capture and recognition remain untested. One initial Wi-Fi reconnection failed and recovered with a controlled reset. The test preserved Wi-Fi settings and SD contents without formatting.
