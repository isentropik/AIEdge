...
| --- | --- | --- |
| Managed OTA staging failure | September 23 authenticated test upload matched the local ZIP hash, but staging returned `storage_error` with a zero-byte `image-archive-settings.js` after earlier files were extracted. Firmware was not installed and boot selection was unchanged. Pending files are retained. Added extraction, integrity, sync and close diagnostics; exact cause remains unresolved. | Diagnose from on-device failure evidence and complete authenticated staging, installation, restart and USB-boundary verification with configuration/password retention. |
