...
| --- | --- | --- |
| Managed OTA staging failure | September 23 authenticated test upload matched the local ZIP hash, but staging returned `storage_error` with a zero-byte `image-archive-settings.js` after earlier files were extracted. Firmware was not installed and boot selection was unchanged. Pending files are retained. An uncompressed ZIP subsequently completed authenticated OTA, restart and password/configuration retention. A second compressed staging-only trial reproduced the failure at the same file; miniz retains file-read error (20), but instrumented reads did not report a short read. That code alone does not establish the cause. Added extraction-phase diagnostics to distinguish allocation, iterator and finalization failures. | Diagnose from on-device failure evidence and complete authenticated staging, installation, restart and USB-boundary verification with configuration/password retention. |
