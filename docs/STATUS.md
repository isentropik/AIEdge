...
Updated September 25, 2026 (Pacific time). **AIEdge runs on the test board, but is
...
## Archive failure visibility — September 25

The test-board status API and archive page now expose a bounded last-upload error
and HTTP status. A repeated synthetic upload trial reports a connection failure
before any receiver TCP connection was observed. Saved-image inference passed all
three runs at 12.05–12.11 seconds, but image delivery did not pass. Archiving is
restored to disabled, configuration/profile/password are preserved, and no camera
capture or production-meter change occurred. See [failure evidence](IMAGE-ARCHIVE-FAILURES.md).
The test bundle contains private replay fixtures; it is not a public release.
