...
## Saved-history display conversion — prepared September 24
...
The camera-independent startup correction is now prepared: shared journal recovery restores compatible display preferences before HTTP routes start, guarded by SD/bundle readiness. Flow reload uses the same helper. Host fixtures cover missing/incompatible profiles, untrusted storage, read failure, malformed and interrupted journals, pending cleanup and unchanged file bytes. Test-board deployment remains pending.
