...
function validateSaved(s){
 if(!s||typeof s.revision!=='string'||!/^[a-f0-9]{64}$/.test(s.revision)||typeof s.model_compatible!=='boolean'||s.activation!=='not_integrated')throw Error('Unsupported settings response');
...
function mount(doc,fetcher){
 ...
 say(p?'Saved details loaded. They are not active; current readings still use the existing calibration.':'Choose the meter type and check its printed units. Nothing has been saved.');
 ...
 try{const s=await request({method:'POST',headers:{'Content-Type':'application/json','X-AIEdge-Revision':revision},body:JSON.stringify(profile)});if(s.status!=='saved_not_active'||s.active_changed!==false)throw Error('Unexpected save response');say('Saved, but not active. Readings and existing totals have not changed.');}
