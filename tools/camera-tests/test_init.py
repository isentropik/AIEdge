...
const int ESP_OK=0,ESP_FAIL=-1,portTICK_PERIOD_MS=1;
const int OV2640_PID=0x26,OV3660_PID=0x3660,OV5640_PID=0x5640;
#define ESP_LOGD(...) ((void)0)
#define ESP_LOGI(...) ((void)0)
#define ESP_LOGE(...) ((void)0)
struct {bool CameraInitSuccessful=true;int CamSensor_id=123,ImageQuality=0,ImageFrameSize=0;} CCstatus;
...
int main(){
 ...
 for(int id:{OV2640_PID,OV3660_PID,OV5640_PID}){
  sensor.id.PID=id;initResult=0;missing=false;assert(c.InitCam()==ESP_OK);assert(CCstatus.CameraInitSuccessful && CCstatus.CamSensor_id==id);
  initResult=0x105;int before=queries;assert(c.InitCam()==0x105);assert(!CCstatus.CameraInitSuccessful && CCstatus.CamSensor_id==0);assert(queries==before);
 }
 initResult=0;missing=true;CCstatus.CameraInitSuccessful=true;assert(c.InitCam()==ESP_FAIL);assert(!CCstatus.CameraInitSuccessful && CCstatus.CamSensor_id==0);
 missing=false;sensor.id.PID=999;assert(c.InitCam()==ESP_FAIL);assert(!CCstatus.CameraInitSuccessful);
 sensor.id.PID=OV2640_PID;assert(c.InitCam()==ESP_OK);assert(CCstatus.CameraInitSuccessful && CCstatus.CamSensor_id==OV2640_PID);
...
with tempfile.TemporaryDirectory(prefix='aiedge-camera-init-') as folder:
 ...
 subprocess.run([sys.executable,'-m','ziglang','c++','-std=c++11','-O2','-UNDEBUG',str(source),'-o',str(exe)],env=env,check=True)
