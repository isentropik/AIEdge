...
message=s.index('Flow not (yet) started: REST API /value not available!')
start=s.rfind('\n',0,message)+1
unavailable=s[start:s.index('    }',message)]
cpp=r"""
#include <string>
#include <iostream>
using esp_err_t=int;
constexpr int ESP_ERR_NOT_FOUND=-77, HTTPD_403_FORBIDDEN=403;
...
int httpd_resp_send_err(httpd_req_t*,int code,const char* text){++sends;lastType=code;body=text;return sendResult;}
...
"""+branch+r"""
return -99;}
int unavailableHandle(httpd_req_t* req){
"""+unavailable+r"""
}
...
for(int failure=0;failure<2;++failure){
 sends=0;body.clear();sendResult=failure?-42:0;
 const int r=unavailableHandle(&req);
 if(r!=sendResult||sends!=1||lastType!=403||body!="Flow not (yet) started: REST API /value not available!"){std::cerr<<"Setup-state response failed: transport="<<sendResult<<" handler="<<r<<"\n";return 1;}
}
std::cout<<"All 18 reading/setup-state response and transport-result cases passed.\n";}
"""
