...
int nvs_erase_key(nvs_handle_t h,const char*k){assert(h==4&&!strcmp(k,"credential"));++erased;return eraseResult;}
...
int main(){NvsBackend b;uint8_t record[credentialBytes]={};
 ...
 for(int e:{0,ESP_ERR_NVS_NOT_FOUND,9}){eraseResult=e;int old=commits;assert(b.eraseCredential()==(e!=9));assert(commits==old+(e!=9));}eraseResult=0;commit=9;assert(!b.eraseCredential());commit=0;
...
for source in [ROOT/'tools/auth-tests/credential_store_test.cpp',ROOT/'tools/auth-tests/website_access_test.cpp',ROOT/'tools/auth-tests/serial_parser_test.cpp',cpp,loader_cpp]:
