...
struct Fake : Backend {
 std::vector<uint8_t> disk; bool readError=false,writeError=false,uncertain=false,corruptReadback=false,randomError=false,deriveError=false,digestError=false;bool eraseError=false,eraseUncertain=false,eraseNoop=false;unsigned salt=0;int writes=0,erases=0;
 ...
 bool eraseCredential() override {++erases;if(eraseError)return false;if(!eraseNoop)disk.clear();return !eraseUncertain;}
